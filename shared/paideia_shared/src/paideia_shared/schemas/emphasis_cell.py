...
class EmphasisCell(BaseModel):
    ...
    Invariant (enforced by V1):
        is_emphasized == (emphasized_class_count == available_class_count
    ...
    Additional constraint (V2): ``emphasized_class_count <= available_class_count``.
    """
    ...
    def _v2_count_consistency(self) -> EmphasisCell:
        """V2: emphasized_class_count must not exceed available_class_count."""
        if self.emphasized_class_count > self.available_class_count:
            raise ValueError(
                f"V2: emphasized_class_count ({self.emphasized_class_count}) > "
    ...
    @model_validator(mode="after")
    def _v1_is_emphasized_consistent(self) -> EmphasisCell:
        """V1: is_emphasized must equal (emphasized==available and available>0)."""
        expected = (
        ...
        if self.is_emphasized != expected:
            raise ValueError(
                f"V1: is_emphasized=={self.is_emphasized} is inconsistent with counts "
