...
def _section_1_overview(
    ...
    return (
        ...
        f"- 명단 외 응답자 (off-roster): {manifest.n_off_roster_respondents}\n"
        f"- factor_scores 미매칭: {manifest.n_unmatched_factor_scores}\n"
        f"- cluster_assignment 미매칭: {manifest.n_unmatched_cluster_assignment}\n"
        f"- 학생지표 미매칭: {manifest.n_unmatched_student_metrics}\n\n"
        "**통계 정의**:\n\n"
