...
### `immersio email` v0.1.1 — 5종 결함 봉합 (spec 007)

v0.1.1 은 v0.1.0 운영 중 발견된 5종 결함 (self-test ↔ cohort 결합 미정의 ·
status priority 미정의 · cleanup-log 누락 · exam_name multi-value · pre-send
재확인 summary) 을 봉합한다. 자세한 사양은
`specs/007-immersio-email-v0-1-1/{spec,plan,quickstart}.md`.

#### v0.1.1 — self-test ↔ cohort 결합 동작 (FR-C01a)

운영자가 옵션 조합의 결과를 예측할 때 참조 (quickstart §4 와 동일 표):

| 명령 옵션 | cohort 좁힘 | 본인 메일함 도달 | 학생 메일함 도달 | csv 기록 status |
|---|---|---|---|---|
| `--cohort all` | 184 | 0 | 184 | `success` / `skipped` |
| `--cohort low_score` | 94 | 0 | 94 | `success` / `skipped` |
| `--self-test 5 --cohort all` | 184 | 5 | 0 | `test_dummy` / `skipped` |
| `--self-test 5 --cohort low_score` | 94 | 5 | 0 | `test_dummy` / `skipped` |
| `--self-test 5` | 184 | 5 | 0 | `test_dummy` / `skipped` |

**핵심 메시지**: `--self-test` 가 붙으면 **학생 메일함에는 절대 도달하지
않는다**. cohort 좁힘은 self-test 대상 모수에만 영향을 주며, 실제 발송
경로는 운영자 본인 메일함으로 강제 라우팅된다.

#### v0.1.1 — retry mode 동작 변화 (v0.1.0 → v0.1.1 의도된 bugfix · FR-C02c)

status priority (`success` > `failed` > `temporary_failure` > `test_dummy` >
`skipped`) 도입으로 *의도된 동작 변화* (사실상 bugfix · spec §5 호환성 표
및 idea 문서 §2 C-2 와 동일 내용):

| 케이스 (학생 1명) | v0.1.0 동작 | v0.1.1 동작 |
|---|---|---|
| (`success` t=10, `skipped` t=20) × DEFAULT | skip | skip (동일) |
| (`success` t=10, `skipped` t=20) × `--retry-skipped` | **재발송** ⚠️ 중복 사고 | **skip** ✓ 안전 |
| (`failed` t=10, `skipped` t=20) × `--retry-skipped` | 발송 | skip — `--retry-failed` 모드 사용 필요 |
| (`failed` t=10, `temporary_failure` t=20) × DEFAULT | 발송 | 발송 (동일) |
| (`test_dummy` t=10, `success` t=20) × DEFAULT | skip | skip (동일) |

`--retry-skipped` 실행 후 발송 보고서 md 에 동작 변화 안내 출력 (FR-C04f
정책 준수):
