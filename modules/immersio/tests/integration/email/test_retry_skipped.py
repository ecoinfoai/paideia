"""Integration test — --retry-skipped only re-tries skipped (T076)."""
...
def _seed_row(sid: str, status: DispatchStatus, error_kind: str = ""):
...
def test_retry_skipped_only_skipped(email_fixture, monkeypatch) -> None:
    """--retry-skipped: only previously skipped students re-tried."""
    ...
    # Only sids[3] + sids[4] (both skipped) re-tried
    assert set(_CountingDispatcher.captured) == {sids[3], sids[4]}
