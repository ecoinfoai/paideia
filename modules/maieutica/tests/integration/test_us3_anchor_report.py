"""T027 (RED) — Integration test: US3 미확인-anchor exclusion + shortfall report.

Exercises ``maieutica.pipeline.build`` end-to-end on a synthetic multi-subsection
chapter where SOME quiz slots' correct-option evidence is NOT a verbatim line of
their ASSIGNED subsection (→ ``status="미확인"``) and the rest ARE (→ ``확인``).

Contract (SC-005 / G3 / FR-015):
- The adopted ``items`` returned by ``build`` have 미확인 0 — every adopted item's
  ``textbook_evidence.status == "확인"``.
- The 미확인 candidates were EXCLUDED (adopted count < generated count).
- The written ``출제품질리포트.md`` carries the 부족분 사유 with "미확인 제외" and a
  count > 0, and the 요청 vs 산출 row reflects M < N.
- The manifest ``quiz_count`` equals ``len(adopted items)``.

The harness mirrors ``test_us1_diversity.py``: it chunks the chapter and runs
``assign_subsections`` up front so each slot's canned correct-option evidence can
be drawn (or deliberately NOT drawn) from its assigned subsection.
"""
...
# Every third slot (assigned order) gets a non-textbook evidence line → 미확인.
_UNCONFIRMED_EVERY = 3
...
def _write_canned_responses(responses_dir: Path) -> int:
    """Write canned responses; some slots get OFF-subsection evidence (→ 미확인).

    Returns the number of slots deliberately made 미확인 (expected exclusions).
    """
    ...
    unconfirmed = 0
    for pos, slot in enumerate(assigned):
        ...
        verbatim = body[idx % len(body)]
        body_cursor[slot.subsection_chunk_id] = idx + 1
        answer_no = ((slot.ordinal - 1) % 5) + 1
        if pos % _UNCONFIRMED_EVERY == 0:
            # Off-subsection sentence — not verbatim in the assigned subsection,
            # so the answer-anchored scoped lookup fails → status="미확인".
            evidence_line = f"이 문장은 교재 어디에도 없는 {slot.ordinal}번 외부 진술이다."
            unconfirmed += 1
        else:
            evidence_line = verbatim
...
def test_us3_unconfirmed_excluded_and_reported(tmp_path: Path) -> None:
    """미확인 anchors excluded (SC-005); 부족분 사유 names 미확인 제외 with a count."""
    ...
    expected_unconfirmed = _write_canned_responses(responses_dir)
    assert expected_unconfirmed > 0, "fixture must seed at least one 미확인 slot"
    ...
    # SC-005 / G3: the adopted set has 미확인 0.
    assert items, "expected at least some adopted items"
    for it in items:
        assert it.textbook_evidence is not None
        assert it.textbook_evidence.status == "확인", it.item_no

    # 미확인 candidates were excluded → adopted count < requested N.
    assert len(items) < _QUIZ_COUNT
    assert len(items) == _QUIZ_COUNT - expected_unconfirmed

    # Report carries the 부족분 사유 naming the 미확인 exclusion with a count > 0,
    # and the 요청 vs 산출 row reflects M < N.
    report = (run_dir / "출제품질리포트.md").read_text(encoding="utf-8")
    assert "## 요청 대비 산출" in report
    assert "## 부족분 사유" in report
    assert "미확인 제외" in report
    assert str(expected_unconfirmed) in report
    assert "⚠️" in report
